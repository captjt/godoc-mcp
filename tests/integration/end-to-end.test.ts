...
describe('End-to-End Integration Tests', () => {
  ...
  describe('Performance characteristics', () => {
    it('should demonstrate cache performance benefits', async () => {
      ...
      // Calculate averages
      const avgNetwork = metrics.networkFetches.reduce((a, b) => a + b, 0) / metrics.networkFetches.length;
      const avgCache = metrics.cacheFetches.reduce((a, b) => a + b, 0) / metrics.cacheFetches.length;
  ...
  describe('Error recovery scenarios', () => {
    it('should gracefully degrade when pkg.go.dev is slow', async () => {
      ...
      await expect(slowFetcher.getPackageDoc('some/other/package'))
        .rejects.toThrow();
